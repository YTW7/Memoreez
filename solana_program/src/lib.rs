...
pub mod todo_app{
    ...
    pub fn add_todo(ctx: Context<AddTodo>, _content: String
     ) -> Result<()> {
     ...
     pub fn mark_todo(ctx: Context<MarkTodo>, todo_idx: u8
     ) -> Result<()> {
          let todo_account = &mut ctx.accounts.todo_account;
          require!(!todo_account.marked, TodoError::AlreadyMarked);//calling error if marking already marked
          // let user_profile = &mut ctx.accounts.user_profile;
     ...
     pub fn remove_todo(ctx: Context<RemoveTodo>, todo_idx: u8
     ) -> Result<()> {
          // let todo_account = &mut ctx.accounts.todo_account;
          let user_profile = &mut ctx.accounts.user_profile;
